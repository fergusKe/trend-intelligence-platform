.PHONY: help \
        p0-up p0-verify p0-stop p0-start \
...
help:                  ## 列出所有指令（依平台底座→分階段流程順序分組）
	@awk 'BEGIN {FS = ":.*## "} \
	     /^##@ / {printf "\n\033[1m%s\033[0m\n", substr($$0, 5)} \
	     /^[a-zA-Z0-9_-]+:.*## / {printf "  \033[36m%-16s\033[0m %s\n", $$1, $$2}' $(MAKEFILE_LIST)

# ─────────────────────────────────────────────────────────────────────────────
##@ P0 平台底座（k8s + ArgoCD + 監控；一切跑在它上面，必先起）
# ─────────────────────────────────────────────────────────────────────────────

p0-up: cluster-up          ## 起平台：kind 叢集 → ArgoCD → root app（之後靠 GitOps 收斂）
p0-verify: verify          ## 平台 smoke（叢集/ArgoCD/核心 app 健康）
p0-stop: cluster-stop      ## 暫停整座叢集（保留狀態）——跑 Ollama/微調等 host 重活前先執行
p0-start: cluster-start    ## 恢復叢集（pod 收斂約 1-3 分鐘）

# ─────────────────────────────────────────────────────────────────────────────
##@ P1 採集→整理→儲存（Airflow + Spark + MinIO/Iceberg + Postgres + dbt）
# ─────────────────────────────────────────────────────────────────────────────

p1-secrets: pipeline-secrets   ## 【前置】佈 secrets（讀 .env 的 YOUTUBE_API_KEY；冪等、不進 git）
p1-up:                         ## 起 P1 服務：ArgoCD 拉回 → 開 airflow/spark auto-sync → 縮非 P1（監控/hello）省記憶體
	kubectl -n argocd scale deploy,statefulset --all --replicas=1 || true
	kubectl -n argocd rollout status deploy/argocd-server --timeout=120s || true
	$(MAKE) demo-p1-up
	$(MAKE) dev-lean-down
	@echo "P1 服務起中，ArgoCD selfHeal 收斂 1-3 分鐘。前置未跑先 make p1-secrets；接著 make p1-run。"
...
argocd-ui:             ## ArgoCD UI：port-forward + 印初始密碼
	@kubectl -n argocd get secret argocd-initial-admin-secret -o jsonpath="{.data.password}" | base64 -d; echo
	kubectl -n argocd port-forward svc/argocd-server 8081:443

pipeline-trigger:      ## 只手動觸發一輪主 DAG（不等結果；驗收用 p1-run）
	kubectl -n airflow exec deploy/airflow-api-server -- airflow dags trigger yt_trending_hourly

dev-lean-down:         ## 縮非必要 app（監控/hello）騰記憶體，保留 P1 的 airflow/spark/postgres/minio。16GB M4 開發預設
...
# ── 以下為 p0-*/p1-* 的底層實作（保留原名向後相容；docs/errata 有引用）───────────
...
cluster-up:
	...
	@echo "Bootstrap 完成。之後由 ArgoCD 收斂（~3-5 分鐘），跑 make p0-verify 驗收。"
...
pipeline-secrets:
	...
	test -n "$$KEY" || { echo "需要 YOUTUBE_API_KEY：填進 .env（見 .env.example）或 make p1-secrets YOUTUBE_API_KEY=<key>"; exit 1; }; \

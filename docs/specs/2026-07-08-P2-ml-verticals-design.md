...
| LLM LoRA 微調（Qwen3-1.7B fp16） | **M4 host**（MPS） | kind 摸不到 Apple GPU（原則核心） |
...
| 項目 | 決定 |
|---|---|
| 預設/後備 | 預設 **Ollama `qwen3:8b`**（host 原生，多語含 zh 強、q4 量化 ~5.2GB RAM）；fallback **Gemini `gemini-2.5-flash`**（`langchain-google-genai`，key 從 `gemini-api` Secret env）。切換：①自動——Ollama 連線錯誤/逾時 30s → 當次 request 改走 Gemini（`provider` 欄如實回報）②手動 A-B——API 參數 `provider: ollama\|gemini` 強制指定。`grade`/`rewrite`/`generate` 三節點共用同一 provider 決策（一次請求一個 provider，A-B 對照才乾淨）。 |
...
| 開放問題 | 決定 |
|---|---|
| 基座 | **`Qwen/Qwen3-1.7B`**。理由：fp16 權重 ~3.4GB + LoRA 訓練啟用值 → 16GB 級 M4 舒適；多語（zh/en/ja/ko 榜單標題全 cover）；llama.cpp 支援 Qwen3 → GGUF/Ollama 出口暢通。淘汰：Qwen3-4B（fp16 ~8GB + activation，訓練吃緊且慢 3×，demo 增益不值）；Llama-3.2-3B（zh 弱於 Qwen）；Phi-4-mini（zh 弱、社群 GGUF 生態較窄）。**fp16 全程，不碰 bitsandbytes**（🔒 CUDA-only）。 |
...
| LoRA 超參 | PEFT `LoraConfig`：**r=16, lora_alpha=32, lora_dropout=0.05, target_modules=["q_proj","k_proj","v_proj","o_proj","gate_proj","up_proj","down_proj"]**（Qwen 全投影面，r=16 對 1.7B 是充足容量）；訓練 **TRL `SFTTrainer`**（completion-only loss——只對 assistant 段算 loss，validated library 不手刻 mask）：lr 2e-4 cosine、3 epochs、per_device_batch 2 × grad_accum 8（等效 16）、max_len 512、fp16 on MPS。`make train-lora`（host）。MLflow：experiment `title_lora`，log 超參/loss 曲線/adapter artifact（數十 MB）。 |

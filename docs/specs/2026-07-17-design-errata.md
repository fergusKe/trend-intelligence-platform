...
| **上一條的隱形延伸：Spark YAML 模板同樣踩 None，但 Jinja StrictUndefined 讓 `(a or b or c)` 也救不了（2026-07-18 實證）** | `resolve_run_anchor` 只套在 Python task；`spark_silver.yaml` 的 SparkApplication `name`/`--date`/`--hour` 仍裸用 `{{ data_interval_start.strftime(...) }}`。手動 run 時 data_interval_start 在**模板 context 是「未定義」（非 None）**，Airflow 模板用 **StrictUndefined**→一被引用即 `UndefinedError: 'data_interval_start' is undefined`（worker pod log 實證）→ operator render application_file 就爆、SparkApplication CR 從沒建出、spark task 每 try 秒掛、4 try 耗盡 failed（排程 run 有 data_interval 故 spark success，只有手動 run 中招→極易誤判成資源/operator flakiness）。**關鍵：`(data_interval_start or logical_date or dag_run.run_after)` 沒用**——StrictUndefined 在第一個 token 就 raise，`or` 輪不到 short-circuit。**解法＝Jinja `default` filter**（`isinstance(x,Undefined)` 先攔截、不觸發 raise，第二參數 `true` 讓 None/falsy 也退）：`data_interval_start \| default(logical_date, true) \| default(dag_run.run_after, true)`，完整鏡射 resolver。**凡 templated k8s manifest（SparkApplication/KPO overrides）用到 data_interval/logical_date 又需支援手動觸發，一律走 `default` filter 鏈、別裸引用也別用 `or`。** |
| **跨 namespace 的 KubernetesPodOperator 需要目標 ns 的 `events` list/watch RBAC，不是只有 pods（2026-07-18 實證，dbt_run/dbt_test 從沒經 DAG 綠過的真凶）** | `airflow-pipeline-runner` Role（data ns，綁 airflow-worker SA）原給了 `pods`/`pods/log`/`sparkapplications`，KPO 能 build/delete dbt pod，但 `await_pod_start → watch_pod_events → list_namespaced_event` 會 **list 目標 ns 的 events 回報排程進度**→缺權限撞 `(403) events is forbidden: ...cannot list resource events in namespace data`（worker pod log 實證）→ operator raise、task failed，**而 dbt pod 當時還在 ContainerCreating、本體其實會成功**（手動 `dbt run` PASS=7、5 表全建），極易誤判成 dbt/OOM/連線問題。**解法**：Role 補 `events: [get,list,watch]`。**凡 KubernetesPodOperator 在非 airflow ns 起 pod，該 ns 的 Role 除 pods/pods/log 外必含 events list/watch**（SparkKubernetesOperator 不中因它監視 CR、不讀 pod events）。 |
| **KubernetesExecutor 的 DAG 在全新叢集預設 paused；手動 trigger 會建 run 但 scheduler 不排 task（ingest 永遠 0/8）** | 全新部署後 `airflow dags trigger` 建了 DAG run，但 DAG `is_paused=True`→task 全不排、卡在無狀態，易誤判成排程/資源卡死。**解法**：驗收/觸發前先 `airflow dags unpause <dag>`（冪等；已納入 `scripts/p1-run.sh`）。**凡 fresh cluster 首次跑 DAG，先 unpause 再 trigger。** |
| **手動 run 與排程 run 同一 logical hour 會共用 deterministic SparkApplication 名而互刪（`delete_stale_sparkapp` 誤傷）** | SparkApplication 名 `yt-silver-YYYYMMDDHH` 只帶小時；在整點附近手動 trigger，手動 run 的 `delete_stale_sparkapp` 會刪掉排程 run 正在跑的同名 CR（反之亦然）→ 排程 run 的 spark 被腰斬 failed。**紀律：驗收用手動 run 時挑「遠離整點」的窗（管線 ~10-15 分，別跨到下個 :00）**，或暫停排程；p1-run.sh 已 pause categories，但 trending 自身的手動×排程仍靠時間窗避開。 |
| **spark task 韌性：deferrable=True 在此 provider(10.17.1)+單節點環境反而更差；正解是 execution_timeout 放寬 + 關非必要服務騰記憶體（別縮 spark）** | 為治 spark task「SparkApplication 已 COMPLETED、task 卻 failed」曾試 `deferrable=True`，實測連 CR 都沒穩定提交（全程 SparkApp=none、task 反覆 up_for_retry），比同步模式更爛→回同步。單節點 8Gi 上 driver+executor 各 1536m（合計 3G）＋ 執行 operator 的 KE worker pod 同節點，executor 常先撞 `FailedScheduling: Insufficient memory` 等記憶體騰出，**`execution_timeout` 10min 太緊會在 executor 還沒排上就逾時→up_for_retry**；放寬到 20min 給排程延遲留餘裕（spark 本體 ~2-3min）。**紀律：單節點跑重 job 前先 `dev-lean-down`（monitoring/ArgoCD/hello 縮 0）騰記憶體，別縮 job 自身資源**（縮 spark 1024/768 反而真 OOM；dbt 是把 limit 加到 1Gi 消 OOM）。 |

...
| 開放問題 | 決定 | 理由 / 淘汰方案 |
|---|---|---|
| 抓幾個地區 | **8 區：`TW,JP,KR,HK,US,GB,SG,AU`**，定義在 `dags/config/pipeline.yaml` 單一真源；dbt `accepted_values` 與其一致性由 pytest 守門（§11） | 範本有 8 區（DAG 預設）vs 12 區（.env/dbt test）漂移——收斂成一處定義。quota 上 12 區也毫無壓力，選 8 是控制每小時 fan-out pod 數與 dashboard 面板密度；加區 = 改一行 YAML。 |
...
| API 金鑰 | k8s Secret `youtube-api`（`airflow` ns），`make pipeline-secrets` 命令式建立（P0 §7 姿態：不進 git），經 Airflow chart 的 `secret:` values 注入所有 task pod 環境變數 | 沿用 P0「第一個真 secret 用 kubectl create secret 起步」的既定策略；不引入 sealed-secrets（要用才加）。 |
| 失敗重試/告警 | Airflow `retries=3` + exponential backoff（quota 錯誤除外，見上）；單一 region 失敗不擋其他 region（動態映射 task 彼此獨立），下游 Spark 用 `trigger_rule="all_done"` 收到什麼算什麼、輸入為空自然 fail；系統性失敗由 freshness PrometheusRule 告警（§9） | mostPopular API 無歷史——某 region-hour 錯過就是永久缺口，重試窗口內救不回就接受缺口，這是**資料源特性**，寫進 README known-limit。 |
...
**Categories 維度**：從 hourly 主線抽離成 `yt_categories_daily`（@daily）——categories 幾乎不變，範本卻每小時抓（浪費 quota）且 bronze 物件按月覆寫、**從未晉升到 silver**（下游 `stg_youtube_categories` 讀空表，範本斷線之一）。新 DAG：抓 8 區 categories → bronze `youtube_categories/region=<XX>/date=<D>/categories.json` → 直接 UPSERT Postgres `silver.youtube_categories`（維度小到不需要過 Spark/Iceberg——刻意決定，寫進 model 文件）。
...
| 項目 | 決定 |
|---|---|
| MinIO 形狀 | 單 replica StatefulSet（`data` ns）+ PVC 10Gi（**無 storageClassName**，kind 用預設 local-path，EKS 換 class 時 values 級改動）+ ClusterIP Service :9000。root 憑證從 Secret `minio-root`（`make pipeline-secrets` 生成隨機密碼）。 |
...
| Iceberg catalog | JDBC catalog：catalog 名 `lakehouse`，backing = Postgres `lakehouse` db（`iceberg_tables` / `iceberg_namespace_properties` 由 Iceberg 自建）。Spark conf：`spark.sql.catalog.lakehouse=org.apache.iceberg.spark.SparkCatalog`、`type=jdbc`、`uri=jdbc:postgresql://lakehouse-postgres.data:5432/lakehouse`、`warehouse=s3a://silver/warehouse` + S3A endpoint/path-style/憑證（env from Secret）。pyiceberg 側用 `sql` catalog 指同一 URI（同表佈局互通；§12 實查 3 落地驗一次）。 |
| Postgres 形狀 | 單 replica StatefulSet（`data` ns）+ PVC 10Gi（無 storageClassName）+ init SQL ConfigMap：建 `airflow`、`lakehouse` 兩個 database；`lakehouse` 內建 `silver`/`gold` schema、`dbt_runner`（讀寫）與 `grafana_reader`（唯讀 gold）角色，密碼取自 Secret 注入的環境變數。**一套 Postgres 三個職責（Airflow metadata / Iceberg catalog / Silver serving+Gold）**——「DB 只 Postgres」的字面實踐；demo 規模無隔離需求，README 註記雲上拆法。 |
...
| SparkApplication 規格 | `mode: cluster`、`type: Python`、driver 1 core/1.5Gi、executor `instances: 1`×1 core/1.5Gi、`restartPolicy: Never`、`timeToLiveSeconds` 清理已完成 app。image = `ghcr.io/<owner>/trend-intelligence-platform/spark-jobs:sha-*`（CI 產物）。模板放 `dags/templates/spark_silver.yaml`，`SparkKubernetesOperator` 提交到 `data` ns 並 watch 到終態；image tag 由 DAG 從 `config/images.yaml` 讀入注入。 |
...
| Silver→Postgres loader | **不是 Spark 的事**：獨立 Airflow task `load_silver_to_postgres`（PythonOperator）用 pyiceberg 掃該小時 snapshot → psycopg2 `INSERT … ON CONFLICT (video_id, region, captured_at) DO UPDATE`（ga4 範本的 UPSERT 冪等模式）。資料量 ~400 列/時，pyiceberg→arrow→executemany 綽綽有餘，不用為 loader 拖 Spark JDBC。首行前 `CREATE TABLE IF NOT EXISTS`（DDL 由 loader 程式碼持有，冪等）。 |
...
| 開放問題 | 決定 |
|---|---|
| dbt 在 k8s 怎麼跑 | **KubernetesPodOperator** 跑自建 dbt image（`lakehouse/dbt/Dockerfile`：`python:3.12-slim` + `dbt-postgres==1.10.2` + 專案本體烤進 image）。兩個 task：`dbt_run`、`dbt_test`（分開讓 DAG 上的 DQ gate 可見；不用 `dbt build` 混流）。淘汰：astronomer-cosmos（把 dbt 圖展開成 Airflow task——多一套依賴與心智模型，5 個 model 用不上）；dbt 裝進 Airflow image（依賴糾纏，違反 task pod 單一職責）。 |
| Gold 的 Postgres | 共用 `data` ns 的 lakehouse Postgres（§4），`lakehouse` db / `gold` schema。profiles：host/密碼走 env（Secret 注入 KPO pod），`threads: 4`。 |
| 分層命名 | 對齊 ga4 範本精神但不照搬四層：**staging（view，`stg_` 前綴）→ marts（table，`gold_` 前綴，schema `gold`）**。中間 bronze/silver 物理層由 MinIO/Iceberg/loader 承擔，dbt 不重複建層（ga4 是 BigQuery 單倉所以四層全在 dbt；本專案分層跨系統，dbt 只管 serving 側）。 |
| sources | **正規 `_sources.yml`**（修範本 hardcode `FROM silver.*` 之弊）：source `silver` = `video_snapshots`、`youtube_categories`，`loaded_at_field: ingested_at` + **freshness（warn 2h / error 4h）**。DAG 內 `dbt_test` 前不單設 freshness task——freshness 檢查併入 `dbt_test` 步（`dbt source freshness` 由 `dbt_test` task 的 command 一起跑）。 |
...
| 排程細節 | 主 DAG `"0 * * * *"`、`max_active_runs=1`（防重疊寫）、task `execution_timeout=10min`、`retries=3`+exponential backoff（quota fail-fast 例外）。 |
...
| `minio-root` | data、airflow | 隨機生成 root user/password（MinIO 本體 + ingest/loader/Spark 用） |
| `lakehouse-postgres` | data、airflow | postgres superuser + `dbt_runner` + `grafana_reader` 密碼 + Airflow metadata connection string（`metadataSecretName` 引用） |
| `grafana-lakehouse-reader` | monitoring | `grafana_reader` 密碼（Grafana datasource 用，§9） |
...
| 開放問題 | 決定 |
|---|---|
| 管線指標怎麼出 | **兩源，各管其分**：①**Airflow 官方路徑** chart `statsd.enabled: true`（statsd-exporter 隨 chart 部署）+ 我方一個 ServiceMonitor 對準 statsd svc → DAG/task 成功失敗、時長等執行面指標。②**資料面指標走 postgres-exporter 自訂查詢**（image v0.20.1，部署在 `lakehouse/postgres/k8s/`）：`yt_freshness_seconds`（`now()-max(ingested_at)`）、`yt_silver_rows_24h{region}`、`yt_gold_mart_rows{mart}`。範本的自訂 `metrics_exporter.py` **不搬**——recon 證實它是從未被呼叫、port 也沒人綁的整組死碼（§10）；資料面真相在 DB 裡，用既有 exporter 的 custom query 拿，不自養 exporter 進程。 |
...
| 5 | Silver（Iceberg + serving） | Postgres：`SELECT count(*) FROM silver.video_snapshots` | > 0，且 `captured_at` 為當前小時 |
...
1. **Airflow chart 1.22.0 的 UI ingress values key**（Airflow 3 將 webserver 併入 api-server，chart 的 ingress 區塊 key 名要對 chart values.yaml 校準）＋ `secret:`/`extraEnvFrom` 注入 task pod 的確切 values 形狀。
2. **spark-operator 2.5.1 chart 產出的 job 側 ServiceAccount 名**（SparkApplication `driver.serviceAccount` 引用它）與 `spark.jobNamespaces` 的確切 values 路徑。
3. **pyiceberg 0.11.1 `sql` catalog 與 Java JDBC catalog 的表佈局互通**：落地先跑一次「Spark 寫 → pyiceberg 讀」煙囪驗證（設計依官方文件判定相容，此為 5 分鐘實證）。
4. **dbt-postgres 1.10.2 解析到的 dbt-core 版本**（`uv pip compile` 一次定 lock，image 內 pin）。

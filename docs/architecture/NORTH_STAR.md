...
2. **一個工作只用一個工具**（亂的真正來源是工具冗餘，不是能力廣度）：
   ...
   - 監控只用 **Prometheus + Grafana**（不要 9 件套 UI 堆疊）。〔**2026-07-10 翻案**：見下〈觀測性三柱翻案〉——補 **Loki（log 柱）＋ Tempo（trace 柱）＋ OTel（儀器化）**，仍守「不要 9 件套」——加的是 **Grafana 原生輕量單體**（Loki/Tempo 皆單 binary、同一 Grafana 讀），**不是 ELK 那種堆疊**〕
...
## 統一資料作品集重定位（2026-07-10 定案，Fergus 令）

> **地位**：本段是「呈現層與部署拓撲」＋「前端說明式 UI」的**上位增補**，屬架構層翻案，**經 Fergus 逐項拍板**（照 CLAUDE.md §卡住/要決策時 已回報並確認）。原「P4 = 平台儀表板」定位保留原文並以本段擴充，留審計軌跡。

### 重定位一句話

前端從「P4 平台儀表板」升為**統一資料作品集**：**一個站、頂層四支柱主題切換，直接展示這個網站即可涵蓋全部求職能力**——🔍**搜尋** ｜ 📊**GA 分析** ｜ 📈**趨勢智能** ｜ 🏗**平台架構**。此站**取代 `llm-workshop/ga-insight`**（GA 分析做進來、ga-insight 退役）、並**納入 `ptt-search` 的搜尋工程展示**。

### 為什麼

三份原本分散的求職作品——ga-insight（GA 分析）、ptt-search（ES 全文檢索）、本平台（DE/MLOps/DevOps）——內容**已在本平台整合**（GA4 自 2026-07-09 起是 P6/P7 的真來源）。收攏成一個可直接展示的站：敘事更強（一個連結講完三種能力）、維護面更少（退役 ga-insight 獨立 app）、視覺統一（共用 Signal 設計系統）。

### 整合模式（鎖定）＝統一靜態站 ＋ 選配 live 連結（守 P4 拓撲鐵律）

- 搜尋與 GA 這兩個原本是 **live 後端**的主題，**重建為本站原生頁、讀平台端預產的靜態 JSON**；真運算/ES 在叢集或離線批次，站上以**預產結果 ＋ 截圖/GIF ＋ MCP 實查**佐證，**每主題可附一個「live demo」外連**（如 ptt-search 已文件化的 Vercel+CloudRun+Bonsai 部署）。
- **否決**：「部分 live 後端」（翻「平台不部署」鐵律、多常駐 infra）、「各自部署 federated 入口」（非真正「一個站」，與「直接展示這個網站就好」相斥）。
- 公開站仍 **`output:'export'` 純靜態、禁 ES/runtime 服務、匯出 JSON 為合約邊界**（P4 鐵律零鬆動）。

### 四支柱定位

| 支柱 | 內容 | 資料/呈現 |
|---|---|---|
| 🔍 **搜尋** | 展示搜尋工程（取材 ptt-search ES 全文檢索敘事） | 站上 = client fuse.js 快速搜 ＋「真 ES 全文檢索」live-demo 外連 ＋ 截圖；**公開靜態站不架 ES** |
| 📊 **GA 分析** | **以「整個銷售漏斗」為核心、比 ga-insight 更完整**的 GA 分析（成長/漏斗/歸因/RFM/LTV…）；**用 Fable 5 規劃我方更完整的功能與流程，參考 ga-insight 但不照抄** | 分析在平台端批次算好（Airflow，唯讀取材 ga-insight `src/analytics/*` 引擎邏輯）→ 匯出靜態 JSON → 前端純讀；跑公開 `ga4_obfuscated_sample_ecommerce` |
| 📈 **趨勢智能** | 現有 11 頁（P4 八頁 ＋ P6 `/reco` ＋ P7 `/audience` ＋ 即時 `/streaming`） | 沿既有 P4/P6/P7 合約，零改 |
| 🏗 **平台架構** | DE/MLOps/DevOps 敘事、架構圖、截圖牆、MCP 指引、誠實敘事 | 沿 P4 `/architecture` 擴充 |

### 設計系統地基（已立）

**Signal 設計系統**（`docs/specs/2026-07-10-frontend-design-system-design.md`，`de3e9d3` 已把關提交）＝四支柱共用視覺地基，**pillar-agnostic**：token/元件/Tailwind v4+shadcn 機制/動效/搜尋/admin 全數成立；本重定位只把其**頂層 IA**（原「11 頁分 5 組 sidebar」）擴為**四支柱主題切換 ＋ 主題內導航**，讓使用者清楚當前在看哪個主題。屬 additive，設計系統本體零重寫。

### 說明式內容體系（對標並超越 ga-insight）

ga-insight 的強項＝**每頁強制自我說明**（`render_page_header` 五欄：章節／問句標題／怎麼看／你能做到的／現在的問題 ＋ 每指標「公式＋實例」inline caption ＋ AI-vs-規則誠實分區）；其弱點＝文案 inline 散落、無集中 registry、無 formula/dataSource/caveats 欄位。**本站超越法**：把說明升為**集中式 registry schema**（keyed by feature id，欄位含 章節/問句標題/怎麼看/gain/pain/**公式/資料源/注意事項**/AI-vs-程式誠實標），跨全站硬性。crosscut 定 schema、各支柱填內容。與既有「說明式 UI 三層元件」（InfoTooltip/ChartCaption/Explainer，正典路徑 `frontend/src/components/explainers/`）銜接：元件是容器、registry 是內容真源。

### 取材與退役（唯讀取材不改原專案，沿可複用素材地圖紀律）

- **ga-insight**（Streamlit）：唯讀取材其分析引擎邏輯（`src/analytics/{rfm,attribution,conversion,predictive,...}`）與五欄自我說明模式後，**退役**（不再獨立維護，能力併入本站 GA 支柱）。**碼不可抄**（Streamlit/Plotly≠Next/Recharts），取的是分析邏輯與內容架構。
- **ptt-search**：保留為搜尋支柱的 **live-demo 部署來源**（維持其 Vercel+CloudRun+Bonsai 部署或截圖），唯讀取材其搜尋 UX 敘事；本站搜尋支柱不重建 ES。

### 待辦序（2026-07-10）

1. **記錄三文件**（本檔 ← 本段即是／`CLAUDE.md`／`README.md`）本重定位。
2. **程式 crosscut**（`superpowers:brainstorming` 交 Fable 5）：定四支柱 IA ＋ 主題切換 UX ＋ 整合模式落法 ＋ **說明式內容 registry schema** ＋ ga-insight/ptt-search 退役/取材接法 ＋ 建置序 ＋ 列後續 spec。
3. **各支柱 spec**（吃 crosscut 合約）：📊 GA 分析支柱（**漏斗為核心**，含平台端分析引擎＋匯出 dataset＋前端頁）、🔍 搜尋支柱（fuse.js 快速搜＋live-demo 外連＋誠實敘事）。
4. **才輪到寫 plan**（先 spec 不開發，延續本輪原則）。

---

## 觀測性三柱翻案（2026-07-10 定案，Fergus 令）

> **地位**：本段是「核心設計原則 §2 監控只用 Prometheus+Grafana」與 P0/P5 觀測性規劃的**上位增補**，屬架構層翻案，**經 Fergus 拍板「全四項」**（照 CLAUDE.md §卡住/要決策時 已回報並確認）。原「監控只用 Prometheus+Grafana」保留原文並標「見本段翻案」，留審計軌跡。full ADR-lite 由觀測性強化 design 展開、收攏進 P5 `DECISIONS.md`。

### 為什麼翻案：平台 metrics 柱已強，缺另兩柱＋修復層

接地盤點（P0/P1/P2/P3/P6/ask-ai design）證實：**指標（metrics）柱規劃已相當完整**——kube-prometheus-stack GitOps、FastAPI instrumentator `/metrics`、postgres-exporter 自訂 DQ 查詢（freshness/drift）、freshness/pipeline/drift 告警、probe 全有。但作為**求職 portfolio 且「跑基建即目的」**，缺口正是資深 DE/SRE 面試最愛問的三塊：**可觀測性三柱缺 log 柱與 trace 柱、且 DQ 只「失敗即擋」無「偵測→修復」層**。補齊＝最高履歷價值，且皆輕量。

### 四項擴充（ADR-lite；前兩項引新工具，後兩項零新常駐服務）

| 項 | 加什麼 | 解決什麼（既有 Prom+Grafana 做不到） | 為何不用替代／誠實護欄 |
|---|---|---|---|
| **① Tracing** | **OpenTelemetry SDK 儀器化** ＋ **Grafana Tempo**（trace 後端）＋ Prometheus **exemplars**（metric↔trace 關聯）。把 ask-ai 現有「LangGraph 節點軌跡存 DB」升級成**真 OTel span**，並跨服務傳播 | 分散式請求追蹤＝Prometheus（指標）/Grafana 都不做；三柱缺一柱 | 不用 Jaeger（Tempo 是 Grafana 原生、單 binary、與現有 Grafana 同棧）；誠實：demo 規模 trace 量小，emptyDir 不留存 |
| **② Logging** | **結構化 JSON log 慣例**（全服務）＋ **Grafana Loki**（單 binary 集中 log） | 集中式/可查詢 log＝Prometheus 不存 log、現況只有分散 pod log | 不用 ELK（正是原「不要 9 件套」拒絕的堆疊）；Loki＝**Grafana 原生輕量單體**、同一 Grafana 查，故「補 log 柱」而非「加 UI 堆疊」 |
| **③ SLO / error budget** | 把散落閾值（freshness>3h、AUC<0.60…）收斂成**正式 SLO ＋ error budget ＋ Grafana SLO 面板** | 現況只有 threshold 告警、無 SLO 物件/錯誤預算/可用性目標 | **零新常駐服務**（純 PrometheusRule + Grafana）；敘事＝「我實踐 SRE SLO」 |
| **④ 自癒/修復層** | pipeline **偵測→修復→標記（was_healed）＋ 四級健康報告**（唯讀取材 CodeWithYu `SelfHealingPipeline` 的 `_heal_review` 應用層 pattern，不改原專案） | 現況 DQ gate 都是「失敗即擋」（dbt test/freshness），無自動修復＋健康分級敘事 | **純應用層、零基建**；最有辨識度的「pipeline 會自己修」敘事 |

### 對「一個工作一個工具」紀律的關係（同三工具翻案的正確套用）

- 原紀律真正目的＝**拒絕冗餘工具**（finmind 教訓），不是拒絕任何新工具。①② 各補一根 Prometheus/Grafana **做不到**的可觀測性柱（trace / log），**非功能重疊的冗餘** → **通過「一工一具」而非違反**。③④ 更是零新常駐服務。
- **仍守的邊界**：Loki/Tempo 皆選 **Grafana 原生單 binary**（不引 ELK、不引 Jaeger＋獨立 UI）；不因此再開第二個 metrics 系統（仍 Prometheus）、第二個儀表板（仍 Grafana）。

### 拓撲誠實（延續 P4 鐵律）

- Loki/Tempo/OTel collector 皆**叢集內服務**，與 Redis/ClickHouse/Flink 同性質——**不是公開靜態站的 runtime 依賴**；以 **Grafana 截圖/GIF ＋ 架構圖 ＋ P5 交付截圖**佐證。前端 🏗平台架構支柱敘事此三柱，**不新增 live 後端**。
- 監控資料延續 P0「emptyDir、demo 重建即重來」姿態（Tempo/Loki 亦不強求持久化，誠實標 known-limit）。
